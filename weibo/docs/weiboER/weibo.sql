...
-- Tue 27 May 2014 10:54:24 PM CST
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_user` (
  ...
  `lock` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否锁定(0:否，1:是）',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_userinfo` (
  ...
  `constellation` CHAR(10) NULL DEFAULT '' COMMENT '星座',
  `intro` VARCHAR(100) NULL DEFAULT '' COMMENT '一句话介绍自己',
  ...
  `style` VARCHAR(45) NOT NULL DEFAULT 'default' COMMENT '个性模板',
  `follow` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关注数',
  `fans` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '粉丝',
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_follow` (
  `follow` INT UNSIGNED NOT NULL COMMENT '关注用户ID',
  `fans` INT UNSIGNED NOT NULL COMMENT '粉丝用户ID',
  `gid` INT NOT NULL COMMENT '所属分组ID',
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_letter` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `from` INT NOT NULL COMMENT '发私信用户ID',
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_weibo` (
  ...
  `isturn` INT NOT NULL DEFAULT 0 COMMENT '是否转发（0：原创；如果是转发的，则保存该转发微博的ID）',
  ...
  `comment` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '评论次数',
  `uid` INT NOT NULL COMMENT '所属用户ID',
  PRIMARY KEY (`id`),
  INDEX `uid` (`uid` ASC))
ENGINE = MyISAM
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_picture` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `mini` VARCHAR(60) NOT NULL DEFAULT '' COMMENT '小图存放地址',
  `medium` VARCHAR(60) NOT NULL DEFAULT '' COMMENT '中图存放地址',
  `max` VARCHAR(60) NOT NULL DEFAULT '' COMMENT '大图存放地址',
...
CREATE TABLE IF NOT EXISTS `weibo`.`hd_keep` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `uid` INT NOT NULL DEFAULT 0 COMMENT '收藏用户的ID',
