...
Class CommonAction extends Action {
	...
	public function _initialize() {
		...
		if(!isset($_SESSION['uid']) && !$this->_post('session_id')) {
			redirect(U('Login/index'));//redirect('Login/index');有时会莫名地出错，故添加U()
		}
	}
	...
	/*public function __construct() {
		parent::__construct();//不继承父类的construct,会有很多方法不能使用，如display()
	...
	public function addFollow () {
		...
		if(M('follow')->data($data)->add()) {
			$db = M('userinfo');
			$db->where(array('uid' => session('uid')))->setInc('follow');//本人的关注数+1
			$db->where(array('uid' => $data['follow']))->setInc('fans');//他人的粉丝数+1
