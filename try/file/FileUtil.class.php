...
class FileUtil {
     ...
     * 遍历文件
     *
     * @param string $dir 需遍历的目录
     * @return array
     */
    function scanFile_1 ($dir) {
        $files = array();
        $dir_list = scandir($dir);
        foreach ($dir_list as $file) {
            if($file != '..' && $file != '.') {
                if(is_dir($dir .'/' . $file)) {
                    $files[$file] = self::scanFile_1($dir . '/' . $file);
                } else {
                    $files[] = $file;
                }
            }
     ...
     * @return array
     */
    function scanFile ($dir, $files=array()) {
        $dir_list = scandir($dir);
        foreach ($dir_list as $file) {
            if($file != '..' && $file != '.') {
                if(is_dir($dir .'/' . $file)) {
                    self::scanFile($dir . '/' . $file, &$files);
                } else {
                    $files[] = $dir .'/' . $file;
                }
            }
     ...
     *
     * @param string $dir 需遍历的目录
     * @param string $type 类型 * 所有; *.php 所有PHP文件...
     * @return array
     */
    function globFile ($dir, $type, $files=array()) {
        $dir_list = glob($dir.$type);
        foreach ($dir_list as $file) {
            if($file != '..' && $file != '.') {
                if(is_dir($file)) {
                    self::globFile($file . '/', $type, &$files);
                } else {
                    $files[] = $file;
                }
            }
